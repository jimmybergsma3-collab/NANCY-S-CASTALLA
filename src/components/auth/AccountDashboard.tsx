...
export function AccountDashboard({ locale }: { locale: Locale }) {
  ...
  return <div className="mt-8 grid gap-6 lg:grid-cols-[300px_1fr]"><aside className="h-fit rounded-lg border border-forest/10 bg-white p-5 shadow-soft"><p className="text-sm text-forest/60">Signed in as</p><p className="mt-1 break-all font-bold text-forest">{email}</p><button className="mt-5 rounded-full border border-forest/20 px-4 py-2 text-sm font-bold text-forest" onClick={logout} type="button">Sign out</button>{resetting ? <form className="mt-6 border-t border-forest/10 pt-5" onSubmit={updatePassword}><label className="text-sm font-bold text-forest">New password<input className="mt-1 w-full rounded-lg border px-3 py-2" minLength={8} onChange={(event) => setPassword(event.target.value)} required type="password" value={password}/></label><button className="mt-3 rounded-full bg-forest px-4 py-2 text-sm font-bold text-cream" type="submit">Update password</button></form> : null}</aside><section><h2 className="font-serif text-3xl font-bold text-forest">Order history</h2>{message ? <p className="mt-4 text-sm text-forest/65">{message}</p> : null}<div className="mt-4 grid gap-3">{orders.map((order) => <article className="rounded-lg border border-forest/10 bg-white p-4 shadow-soft" key={order.id}><div className="flex flex-wrap items-center justify-between gap-3"><strong>{order.order_number ? `NC-${String(order.order_number).padStart(6, "0")}` : order.id}</strong><strong>€{Number(order.total).toFixed(2)}</strong></div><p className="mt-2 text-sm text-forest/65">{new Date(order.created_at).toLocaleDateString()} · {order.status.replaceAll("_", " ")} · {order.payment_status}</p></article>)}{!message && orders.length === 0 ? <p className="text-sm text-forest/65">No orders yet.</p> : null}</div></section></div>;
}
