  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_390px]">
      ...
      <aside className="h-fit rounded-lg border border-forest/10 bg-cream p-5 shadow-soft lg:sticky lg:top-32">
        ...
        <form className="mt-5 space-y-3" onSubmit={submitOrder}><input className="w-full rounded-lg border border-forest/15 bg-white px-3 py-2 text-sm" onChange={(event) => setCustomerName(event.target.value)} placeholder={copy.name} required value={customerName} /><input className="w-full rounded-lg border border-forest/15 bg-white px-3 py-2 text-sm" onChange={(event) => setCustomerEmail(event.target.value)} placeholder={copy.email} required type="email" value={customerEmail} /><input autoComplete="tel" className="w-full rounded-lg border border-forest/15 bg-white px-3 py-2 text-sm" inputMode="tel" onBlur={() => setCustomerPhone(formatCustomerPhone(customerPhone))} onChange={(event) => setCustomerPhone(event.target.value)} placeholder={copy.phone} type="tel" value={customerPhone} /><textarea className="min-h-20 w-full rounded-lg border border-forest/15 bg-white px-3 py-2 text-sm" onChange={(event) => setCustomerAddress(event.target.value)} placeholder={copy.address} value={customerAddress} /><textarea className="min-h-20 w-full rounded-lg border border-forest/15 bg-white px-3 py-2 text-sm" onChange={(event) => setNotes(event.target.value)} placeholder={copy.notes} value={notes} /><button className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-forest px-5 py-3 text-sm font-bold text-cream disabled:opacity-40" disabled={!canCheckout || status === "sending"} type="submit"><Send size={17} />{status === "sending" ? copy.sending : copy.send}</button></form>
