...
export async function GET(request: Request) {
  ...
  const orders = await supabaseAdminFetch<Array<Record<string, unknown>>>(`orders?select=id,order_number,created_at,status,payment_status,payment_method,fulfillment,delivery_method,notes,subtotal_ex_vat,vat_total,total,order_items(*),invoices(id,invoice_number,status,email_sent_at,issued_at)&customer_id=eq.${customers[0].id}&order=created_at.desc&limit=100`);
