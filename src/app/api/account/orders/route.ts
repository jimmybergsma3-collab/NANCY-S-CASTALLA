...
export async function GET(request: Request) {
  ...
  const orders = await supabaseAdminFetch<Array<Record<string, unknown>>>(`orders?select=id,order_number,created_at,status,payment_status,total&customer_id=eq.${customers[0].id}&order=created_at.desc&limit=100`);
