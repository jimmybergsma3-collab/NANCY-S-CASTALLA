...
import { getPublicProductDescription, getPublicProductDetailText } from "@/lib/product-display";
...
export async function generateMetadata({ params }: { params: Promise<unknown> }): Promise<Metadata> {
  const { locale: rawLocale, productId: rawProductId } = (await params) as { locale?: string; productId?: string };
  ...
  const description = getPublicProductDescription(product, locale);
  const productName = translateProductName(product.name, locale);
  const images = product.imageUrl ? [{ url: product.imageUrl, alt: productName }] : undefined;
...
function getPublicAdditionalInfo(product: {
  ...
  unit: string;
  origin: string;
} & Parameters<typeof getCustomerDisplayUnit>[0], soldAs: string, originLabel: string, locale: Locale) {
  const info = product.additionalInfo?.trim() ?? "";
  const supplierNote = /^(supplier section|supplier category|dutch name):/i.test(info);

  if (locale === "en" && info && !supplierNote) {
    return info;
  }
    ...
    <section className="mx-auto max-w-7xl px-4 py-12">
      ...
      <div className="mt-6 grid gap-10 lg:grid-cols-[minmax(0,1fr)_460px]">
        ...
        <div>
          ...
          <h1 className="mt-2 font-serif text-5xl font-bold text-forest">{productName}</h1>
          <div className="mt-3 text-2xl font-bold text-forest">
            {displayedPrice > 0 ? formatEuro(displayedPrice) : dictionary.common.soon}
          </div>
          {product.isVisible !== false ? (
            <ProductShareButton locale={locale} productCode={product.id} productName={productName} />
          ) : null}
          <p className="mt-5 leading-7 text-forest/72">{getPublicProductDescription(product, locale)}</p>
          ...
          <div className="mt-6 border-b border-forest/15">
            {product.ingredients ? <DetailPanel open title={ui.products.ingredients}>{getPublicProductDetailText(product.ingredients, locale)}</DetailPanel> : null}
            {product.directions ? <DetailPanel title={ui.products.directions}>{getPublicProductDetailText(product.directions, locale)}</DetailPanel> : null}
            {product.conservation ? <DetailPanel title={ui.products.conservation}>{getPublicProductDetailText(product.conservation, locale)}</DetailPanel> : null}
            <DetailPanel title={ui.products.additionalInformation}>
              {getPublicAdditionalInfo(product, ui.products.soldAs, ui.products.origin, locale)}
            </DetailPanel>
          </div>
        </div>
      </div>
      <div className="mt-10">
        <ProductOrder hideCardImages initialCategory={product.category} locale={locale} products={[product]} />
      </div>
        ...
        <div className="mt-14">
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {relatedProducts.map((item) => {
              const itemName = translateProductName(item.name, locale);
              return (
                <Link
                ...
                >
                  ...
                  <h3 className="mt-4 min-h-12 text-sm font-bold text-forest">{itemName}</h3>
