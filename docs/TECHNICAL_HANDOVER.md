...
| Facturatie | Werkend na migratie | Orderfactuur, losse admin/passantenfactuur met optionele bestaande klantkoppeling en productzoeker, snapshots, PDF, klantdownload en Resend-verzending; nog geen creditnota of boekhoudexport |
...
`GET` geeft de factuurlijst met regels terug. `POST` ondersteunt `create` vanuit een order, `email` via Resend, markeren als test en archiveren/herstellen. Alle acties vereisen een adminsessie. Creatie gebruikt `create_invoice_from_order`; mailfalen verandert of verwijdert de factuur niet. Een voor correctie ingetrokken factuur blijft bestaan met status `void`, `voided_at`, `voided_by`, `void_reason` en onveranderde `invoice_items`; de snapshot staat ook in auditmetadata en de admin maakt na correctie een nieuwe factuur met nieuw nummer.
