...
begin
  ...
  if p_status = 'confirmed' and not v_order.inventory_committed then
    for v_item in select * from order_items where order_id = p_order_id loop
      ...
      if v_product.track_inventory and v_product.stock_quantity < v_units then
      ...
      if v_product.track_inventory then
        ...
        insert into inventory_movements(product_id, order_id, movement_type, quantity, reference) values(v_product.id, p_order_id, 'sale', -v_units, 'Order ' || coalesce(v_order.order_number::text, p_order_id));
      end if;
    end loop;
    ...
    for v_item in select * from order_items where order_id = p_order_id loop
      ...
      if v_product.track_inventory then
        update products set stock_quantity = stock_quantity + v_units where id = v_product.id;
        insert into inventory_movements(product_id, order_id, movement_type, quantity, reference) values(v_product.id, p_order_id, 'cancellation', v_units, 'Cancelled order ' || coalesce(v_order.order_number::text, p_order_id));
      end if;
    end loop;
