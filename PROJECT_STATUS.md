...
- Volledig productbeheer: toevoegen, wijzigen, verwijderen, publiceren, zoeken, filteren, prijzen, IVA, verpakking, voorraad en afbeeldingen. Op mobiel is er een compacte `Snel product toevoegen`-drawer voor direct handmatig toevoegen met foto, prijs, IVA, categorie en concept/online-keuze.
...
Op 13 juli 2026 is het bestaande productbeheer uitgebreid met mobiele snelle productinvoer op `/{locale}/admin/products`. De drawer gebruikt dezelfde products-tabel, foto-upload en product-create API, zonder databasewijzigingen. Producten kunnen als draft/onzichtbaar worden opgeslagen of direct online wanneer naam, verpakking, verkoopprijs, IVA, categorie, foto, sales unit en prijsbasis compleet zijn. `npm run lint` en `npm run build` zijn succesvol uitgevoerd.
